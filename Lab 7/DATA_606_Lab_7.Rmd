...
```{r load-libraries, warn = F, message = F}
library(tidyverse)
library(ggplot2)
```
...
```
Based on the scatter plot, the relationship between `at_bats` and `runs` does appear to be linear, and I would use a linear model to predict the number of runs.
...
### Solution
There is a positive correlation between number of at bats and number of runs, with a few outliers. 
...
summary(m2)
```
The equation for the regression line is $\hat{y} = 415.2389 + 1.8345 * \text{homeruns}$.
There is a strong correlation between homeruns and number of runs; the more homeruns, the more runs.
...
```
The manager overpredicted, with a residual of `r runs.residual`.
...
### Solution
There is no apparent pattern, and the residuals are centered around the center (0), which would imply a linear relationship. 
...
### Solution
Both plots have a slight skew, but there are enough samples to assume nearly normal.
...
### Solution
We'll use the variable `hits`; the more hits, statisticall, there should be more runs.
```{r question-nine}
...
summary(m3)
```
The $R^2$ for `at_bats` is 0.3729, while the $R^2$ for `hits` is 0.6419, which tells us that `hits` is a better predictor of runs than `at_bats`.
...
mdf
```
`r m4$call$formula`: `bat_avg` is the best predictor for the number of `runs`.
...
mdf.2
```
`new_obs` is the best predictor, which makes sense: number of types getting a base hit, plus slugging efficiency would be a good predictor.
